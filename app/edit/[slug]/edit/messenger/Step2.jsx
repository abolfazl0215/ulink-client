...
const Step2 = ({
  ...
  const [title, setTitle] = useState("messengers");
  ...
  const setTextHandler = (e, id) => {
    // const dataCopy = [...data];
    // const filteredData = dataCopy.filter((f) => f.id === id);
            ...
            {showSelectSection && showSelectSection[d.id] ? (
              <div className={styles.formBlockBody}>
                ...
                {d.social === "whatsapp" ? (
                  <>
                    <label>
                      Predefined WhatsApp Message (optional)
                    </label>
                    <input
                      type="url"
                      onChange={(e) => setMessageHandler(e, d.id)}
                      value={d.message}
                      aria-multiline="true"
                    />
                    <p>
                      Filling this field will set the default text in
                      the WhatsApp chat
                    </p>
                  </>
                ) : (
